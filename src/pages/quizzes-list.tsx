...
import { useQuizzesList, useRetryQuiz } from '@/lib/api/hooks'
import { timeAgo } from '@/lib/time'
import { Link } from '@tanstack/react-router'
import { AlertCircle, ArrowLeft, ChevronRight, ClipboardList, Clock, Loader2, Play } from 'lucide-react'
import { haptics } from '@/lib/haptics'

export default function QuizzesListPage() {
  const list = useQuizzesList()
  const retry = useRetryQuiz()
    ...
    <main className="flex flex-1 flex-col overflow-hidden">
      <header className="sticky top-0 z-10 flex shrink-0 items-center gap-3 border-b bg-background/90 px-4 py-3 backdrop-blur-sm">
        ...
        <div className="flex min-w-0 flex-col gap-0.5">
          ...
          <p className="truncate text-xs text-muted-foreground">Pick a quiz to test yourself</p>
        </div>
      </header>
      ...
      >
        <div className="mx-auto flex w-full max-w-lg flex-col gap-3">
          ...
          {!list.isLoading && items.length === 0 && (
            <EmptyState
              icon={ClipboardList}
              message="No quizzes yet. Generate one to test yourself."
            />
          )}
                ...
                <Card key={`p-${item.id}`} variant="elevated" className="animate-pulse">
                  <CardContent className="flex items-center gap-2 py-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xxs text-muted-foreground">{timeAgo(item.createdAt)}</p>
                    </div>
