    ...
    <main className="flex flex-1 flex-col overflow-hidden">
      <header className="flex shrink-0 items-center gap-3 border-b px-4 py-3">
        ...
        <div className="flex min-w-0 flex-col gap-0.5">
          <h1 className="font-heading text-sm font-medium">Notes</h1>
          <p className="truncate text-[10px] text-muted-foreground">Pick a study guide to read</p>
        </div>
      </header>
        ...
        <div className="mx-auto flex w-full max-w-lg flex-col gap-3">
          ...
          {!list.isLoading && items.length === 0 && (
            <p className="text-center text-xs text-muted-foreground">
              No notes yet. Generate some from the home screen.
            </p>
          )}
            ...
            return (
              <Card key={item.id}>
                ...
                <CardContent className="flex items-center justify-between">
                  ...
                  <Button size="sm" render={<Link to="/notes/$id" params={{ id: item.id }} />}>
                    <BookText />
                    Read
                  </Button>
                </CardContent>
